...
def scrape_top_250(url):
    ...
    '''
    top_250_dom = DOM(url.download(cached=True))

    movie_urls = []

    for movie in top_250_dom('td.titleColumn a'):

        link = abs(movie.attributes.get('href',''), base=url.string)
...
def scrape_movie_page(dom):
    ...
    title = dom('td#overview-top')[0].by_tag('span.itemprop')[0].content

    # strips whitespace and 'mins'
    duration = dom('time')[0].content
    duration = duration.strip()[:-4]

    # separator for genres, directors, writers and actors
    s = ';'

    # saves genres in list and joins them with ; character
    genres = []
    for g in dom('td#overview-top')[0].by_attr(itemprop="genre"):
        genre = g.content
        genres.append(genre)
    genres = s.join(genres)

    directors = []
    for d in dom('td#overview-top')[0].by_attr(itemprop="director")[0].by_attr(itemprop="name"):
        director = d.content
        directors.append(director)
    directors = s.join(directors)

    writers = []
    for w in dom('td#overview-top')[0].by_attr(itemprop="creator")[0].by_attr(itemprop="name"):
        writer = w.content
        writers.append(writer)
    writers = s.join(writers)

    actors = []
    for a in dom('td#overview-top')[0].by_attr(itemprop="actors")[0].by_attr(itemprop="name"):
        actor = a.content
        actors.append(actor)
    actors = s.join(actors)

    #removes whitespace in beginning
    rating = dom.by_class('titlePageSprite star-box-giga-star')[0].content[1:]
